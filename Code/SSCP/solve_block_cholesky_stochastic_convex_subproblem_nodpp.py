...
def solve_block_cholesky_stochastic_convex_subproblem(opt_prob, cvx_prob, ptr_ops, x_ref, u_ref, K_ref):
    ...
    nc_count = 0
    for k in range(N):
        BK = np.zeros((neps, neps))
        A_m = np.eye(neps)
        for m in range(k):
            j = k - 1 - m
            BK = BK + A_m @ opt_prob.disc["B_err_k"][:, :, j] @ K[:, :, j]
            A_m = opt_prob.disc["A_err_k"][:, :, j] @ A_m
                    
        S_x_k = (np.eye(neps) + BK) @ S_unc[:, :, k]
    ...
    K_sol = cvx_prob.var_dict["K"].value
    ...
    for k in range(N - 1):
        S_u_sol[:, :, k] = K_sol[:, :, k] @ S_unc[:, :, k]
        S_u_ref[:, :, k] = K_ref[:, :, k] @ S_unc[:, :, k]
    ...
    return x_sol, u_sol, K_sol, [], sol_info
