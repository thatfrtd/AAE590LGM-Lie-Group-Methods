function [x_sol, u_sol, X_sol, S_sol, sol_info] = solve_stochastic_ptr_convex_subproblem_no_p_noest_noG(prob, ptr_ops, x_ref, u_ref, X_k_ref, S_k_ref)
...
cvx_begin
    ...
    variable v_NP(1,1)
    variable X_C(prob.n.x, prob.n.x, prob.N) % This is for the covariance propoagation. This mf changes dimensions every iteration
    variable S_k(prob.n.u, prob.n.x, prob.Nu)
    ...
    subject to
        ...
        for k = 1:(prob.N - 1)
            ...
            X_C(:, :, k + 1) == prob.disc.A_k(:, :, k) * X_C(:, :, k) + prob.disc.B_k(:, :, k) * S_k(:, :, k);
        end
        % Constraints
        for k = 1:prob.Nu
            % Convex Constraints
            for cc = 1:prob.n.cvx
                prob.convex_constraints{cc}(prob.unscale_x(X(:, k)), prob.unscale_u(U(:, k)), 0, X_C(:, :, k), S_k(:, :, k)) <= 0;
            end
            % Nonconvex Constraints
            for nc = 1:prob.n.ncvx
                prob.nonconvex_constraints{nc}(prob.unscale_x(X(:, k)), prob.unscale_u(U(:, k)), 0, X_C(:, :, k), S_k(:, :, k), x_ref, u_ref, 0, X_k_ref, S_k_ref, k) ...
                    - v_prime(nc) <= 0;
            end
        end
        v_prime >= 0;
        ...
        sqrtm(prob.Phat0) == X_C(:, :, 1); % Initial estimated state covariance
        prob.terminal_bc(prob.unscale_x(X(:, prob.N)), 0) + v_N == 0; % Final mean state
        norm(sqrtm(prob.Pf - prob.disc.Ptilde_k(:, :, end)) \ X_C(:, :, end)) - 1 - v_NP <= 0; % Final estimated state covariance
...
sol_info.vbc_0 = v_0;
sol_info.vbc_N = v_N;
sol_info.vbc_NP = norm(sqrtm(prob.Pf - prob.disc.Ptilde_k(:, :, end)) \ X_sol(:, :, end)) - 1;
