...
#q_0 = np.array([0.291, -0.567, -0.2, 0.743]).reshape((4, 1))#q_exp(theta_0).reshape((4, 1))
...
theta_std = np.deg2rad(np.array([[3], [3], [3]]))
w_std = np.deg2rad(np.array([[3], [3], [3]])) / 2
...
initial_bc = {"x":lambda x, x_ref : x - x_0.flatten(), 
              "S":lambda S : S - np.linalg.cholesky(P_0)}
...
P_tilde = np.diag(np.concatenate((r_std, v_std, theta_std, w_std, m_std)).flatten() / 30) ** 2
S_tilde = np.linalg.cholesky(P_tilde)
...
glideslope_constraint = {"k":np.arange(N), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(x[0:2]) - x[2] * np.tan(glideslope_max_angle)}
glideslope_constraint_chance = {"k":np.arange(1, N - 1), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(x[0:2]) + sqx2inv(0.95, 2) * cp.norm2(S_x[0:2, :]) - (x[2] - sqx2inv(0.95, 1) * cp.norm2(S[2, :])) * np.tan(glideslope_max_angle)}
angvel_max = np.deg2rad(20)
angvel_constraint = {"k":np.arange(N), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(x[10:13]) - angvel_max}
angvel_constraint_chance = {"k":np.arange(1, N - 1), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(x[10:13]) + sqx2inv(0.95, 3) * cp.norm2(S_x[9:12, :]) - angvel_max}
angvel_max_STC = np.deg2rad(2)
angvel_constraint_STC = {"k":np.arange(N), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(x[10:13]) - angvel_max_STC, "trigger":LoS_distance_trigger}
angvel_constraint_STC_chance = {"k":np.arange(1, N - 1), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(x[10:13]) + sqx2inv(0.95, 3) * cp.norm2(S_x[9:12, :]) - angvel_max_STC, "trigger":LoS_distance_trigger}
...
S_x_max_constraint_row = {"k":np.arange(1, N - 1), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(S_x[:-1, :], 1) - max_x_std}
S_x_max_constraint = {"k":np.arange(1, N - 1), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(cp.diag(cp.vstack([1 / max_x_std.reshape((-1, 1)), [0]])) @ S_x) - 1}
state_convex_constraints = [glideslope_constraint, angvel_constraint, S_x_max_constraint]

# Control convex constraints
thrust_max_constraint = {"k":np.arange(Nu), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(u[0:3]) - T_max}
thrust_max_constraint_chance = {"k":np.arange(Nu), "type":"<=", "func":lambda x, u, S_x, S_u : cp.norm2(u[0:3]) + sqx2inv(0.95, 3) * cp.norm2(S_u[0:3, :]) - T_max}
gimbal_constraint = {"k":np.arange(Nu), "type":"<=", "func":lambda x, u, S_x, S_u : 1e3*(cp.norm2(u[1:3]) - u[0] * np.tan(gimbal_max_angle))}
gimbal_constraint_chance = {"k":np.arange(Nu), "type":"<=", "func":lambda x, u, S_x, S_u : 1e3*(cp.norm2(u[1:3]) + sqx2inv(0.95, 2) * cp.norm2(S_u[1:3, :]) - (u[0] - sqx2inv(0.95, 1) * cp.norm2(S_u[0, :])) * np.tan(gimbal_max_angle))}
max_torque_constraint = {"k":np.arange(Nu), "type":"<=", "func":lambda x, u, S_x, S_u : cp.abs(u[3]) - tau_max}
bad_thrust_min_constraint = {"k":np.arange(Nu), "type":"<=", "func":lambda x, u, S_x, S_u : T_min - u[0]} # Replace with nonconvex one? need to be able to nonconvex anyway...
...
control_convex_constraints = [max_torque_constraint, thrust_max_constraint_chance, gimbal_constraint, gimbal_constraint_chance, bad_thrust_min_constraint_chance]
...
for k in range(N):
    if k < Nu:
        u_cov_mag[k] = np.linalg.norm(ptr_sol["S_u"][:, :, k], 2)

plt.plot(sqx2inv(0.99, 4) * u_cov_mag, label = "Mag")
plt.legend()
plt.grid()
...
plot_rocket_landing_6DoF_trajectory(t_k, guess["x"], guess["u"], glideslope_max_angle, gimbal_max_angle, T_min, T_max, p_b, r_LoS_min, r_LoS_max, "Guess")
...
plt.plot(t_cont, np.rad2deg(np.acos(guess["x"][2, :] / np.linalg.norm(guess["x"][0:3, :], 2, 0))))
...
plt.stairs(np.linalg.norm(guess["u"][0:3, 0:Nu], 2, 0) / T_max, t_cont)
...
plt.stairs(np.rad2deg(np.acos(guess["u"][0, 0:Nu] / np.linalg.norm(guess["u"][0:3, 0:Nu], 2, 0))), t_cont)
...
LoS_vals_guess = np.squeeze(np.rad2deg(np.acos([-(np.array(quat_rot(cas.DM(guess["x"][6:10, k]), cas.DM(guess["x"][0:3, k]))).T @ p_b + d_b.T @ p_b) / np.linalg.norm(guess["x"][0:3, k]) for k in range(N)])))
plt.plot(LoS_vals, np.linalg.norm(guess["x"][0:3, :], 2, 0)*1e3)
...
plt.plot(np.rad2deg(np.linalg.norm(guess["x"][10:13, :], 2, 0)), np.linalg.norm(guess["x"][0:3, :], 2, 0)*1e3)
