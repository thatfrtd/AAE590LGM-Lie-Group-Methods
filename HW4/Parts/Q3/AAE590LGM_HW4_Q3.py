...
# Problem 3: Error-State EKF in the BodyFrame
...
def ESKF_SE2(prob):
    ...
    def F_func(x_k, xi_k):
        A = np.array([[1, xi_k[2] * prob.delta_t, 0],
                      [-xi_k[2] * prob.delta_t, 1, xi_k[0] * prob.delta_t],
                      [0, 0, 1]])
        return A
    
    def x_update_func(x_k_km1, delta_x_k):
        x_k_k = np.zeros([3, 1])
        x_k_k[0:2] = (x_k_km1[0:2] + so2_exp(x_k_km1[2]) @ delta_x_k[0:2]).reshape((2, 1))
        x_k_k[2] = x_k_km1[2] + delta_x_k[2]

        return x_k_k

    def x_update_func_good(X_k_km1, delta_x_k):
        X_k_k = se2_mat_to_tuplevec(se2_compose(se2_tuplevec_to_mat(X_k_km1), se2_exp(delta_x_k)))
        return X_k_k
    ...
    def f_disc_func(X_k, xi_k):
        X_kp1 = se2_mat_to_tuplevec(se2_compose(se2_tuplevec_to_mat(X_k), se2_exp(xi_k * prob.delta_t)))
        return X_kp1
    ...
    H_GPS_func = lambda x : np.hstack((so2_exp(x[2]), np.zeros((2, 1))))
...
x_true, X_true, xhat_k, Xhat_k, Phat_k, nu_k, K_k, innovation, t_k_GPS, z_k_GPS, k_GPS = ESKF_SE2(prob)
...
# 3f) Monte Carlo Evaluation
nu_k_runs = np.zeros((N_runs, t_k_GPS.size))
eps_k_runs = np.zeros((N_runs, prob.N_nodes))
for i in range(N_runs):
    x_true, X_true, xhat_k, Xhat_k, Phat_k, nu_k_runs[i, :], K_k, innovation, t_k_GPS, z_k_GPS, k_GPS = ESKF_SE2(prob)
    ...
    '''for k in range(2, prob.N_nodes - 3):
...
plt.savefig("./HW4/Parts/Q3/AAE590LGM_HW4_Q3f_mc.png")
