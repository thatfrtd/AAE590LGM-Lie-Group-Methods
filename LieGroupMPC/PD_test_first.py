...
X_bar_nou = prob.simulate(np.zeros([3, prob.N_steps]), lambda xi_e, eta, xi, xi_bar, k : np.zeros([3, 1]), X_ref, prob.X_0)

K_p = 3e-1 * np.eye(3)
K_d = 0 * np.eye(3)
PD_func = lambda xi_e, eta, xi, xi_bar, k : prob.PD(xi_e, eta, xi, xi_bar, K_p, K_d)
dyn_inv_func = lambda xi_e, eta, xi, xi_bar, k : prob.log_linear_dynamic_inversion(xi_e, xi_bar, K_p)
...
# Plot
for i in np.arange(1, prob.N_nodes, 20):
    error = np.zeros((3, N_runs))
