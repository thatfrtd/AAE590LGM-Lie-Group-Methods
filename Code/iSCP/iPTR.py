...
import time
from Code.iSCP.solve_geodesic_convex_subproblem import solve_geodesic_convex_subproblem

def iPTR(opt_prob, ptr_ops):
    '''
    Penalized Trust Region Sequential Convex Programming algorithm on Lie Groups
    
    Only set up for ZOH discretization right now

    :param opt_prob: DeterministicLieProblem object that describes optimization problem
    :param ptr_ops: {iter_min, iter_max, solver, defect_tol, delta_tol, w_tr, w_vc, alpha_x, alpha_u}
    '''
    
    ## Initial Discretization
    defect_k = np.zeros((opt_prob.n["eps"], opt_prob.n["N"] + 1, ptr_ops["iter_max"] + 1))
    ...
    eps = np.zeros((opt_prob.n["eps"], opt_prob.n["N"], ptr_ops["iter_max"]))
    ...
    while i < ptr_ops["iter_min"] or (i < ptr_ops["iter_max"] and not converged):
        ...
        x[:, :, i + 1], eps[:, :, i], u[:, :, i + 1], cvx_prob, info_i = solve_geodesic_convex_subproblem(opt_prob, cvx_prob, ptr_ops, x[:, :, i], u[:, :, i])
        info.append(info_i)
    ...
    ptr_sol = {"x":x, "eps":eps, "u":u, "info":info, "converged":converged, "converged_i":i, "delta":delta, "defect":defect, "defect_k":defect_k}
