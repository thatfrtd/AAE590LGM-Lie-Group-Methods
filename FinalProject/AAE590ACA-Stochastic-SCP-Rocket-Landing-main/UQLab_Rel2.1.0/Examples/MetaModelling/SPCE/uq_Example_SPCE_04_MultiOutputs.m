%% STOCHASTIC POLYNOMIAL CHAOS EXPANSIONS - MULTIPLE OUTPUTS
%
...
for oo = 1:mySPCE.Internal.Runtime.Nout
    for ix = 1:size(X4plot,1)
        uq_figure('name',sprintf('Output #%i',oo))
        uq_histogram(squeeze(Y4plotRef(ix,oo,:)),'normalized','pdf');
        hold on
        uq_histogram(squeeze(Y4plotSPCE(ix,oo,:)),'normalized','pdf','FaceAlpha',0.5);
        hold off
