...
initial_bc = {"x":lambda x, x_ref : x - x_0.flatten(), 
              "S":lambda S : S - np.linalg.cholesky(P_0) / 3}
...
objective = lambda x, u, S_u, x_ref, u_ref : cp.sum(cp.norm2(u[0:2]) + sqx2inv(0.99, 2) * cp.norm2(S_u)) * delta_t
...
# Combine guesses
guess = {"x":sol_deterministic["x"], "u":sol_deterministic["u"]}
