...
def solve_block_cholesky_stochastic_convex_subproblem(opt_prob, cvx_prob, ptr_ops, x_ref, u_ref, K_ref):
    '''
    ...
    if str(type(cvx_prob)) == "<class 'list'>":
        ...
        K = cp.Variable((nu * neps, N - 1), name = 'K')
        I_BK = cp.Variable((neps * neps, N - 1), name = 'I_BK')
        ...
        S_unc_param = cp.Parameter((neps * int((neps + 1) / 2), N - 1), name = "S_unc")
        B_err_param = cp.Parameter((neps * nu, N - 1), name = "B_err_k")
        ...
        for k in range(N):
            if k < N - 1:
                ...
                K_k = K[:, k].reshape((nu, neps), 'C')
                B_err_k_param = B_err_param[:, k].reshape((neps, nu), 'C')
                I_BK_k = I_BK[:, k].reshape((neps, neps), 'C')

                stoch_constraints.append(I_BK_k == np.eye(neps) + B_err_k_param @ K_k)
                if k > 0:
                    S_unc_k = tril_to_mat_cp(S_unc_param[:, k - 1])
                    S_x_k = I_BK_k @ S_unc_k
                    S_u_k = K_k @ S_unc_k
                else:
                    S_unc_k = np.linalg.cholesky(opt_prob.P_0)
                    S_x_k = I_BK_k @ S_unc_k
                    S_u_k = K_k @ S_unc_k
        ...
        constraints = dynamics_constraints + \
                        ...
                        nonconvex_constraints + \
                        trust_region_constraints + [
                        opt_prob.initial_bc["x"](X[:, 0], x_ref_param[:, 0]) == v_0, # Initial condition constraint
                        opt_prob.terminal_bc["x"](X[:, -1], x_ref_param[:, -1]) == v_N] # Terminal condition constraint
    ...
    S_unc = np.zeros((neps * int((neps + 1) / 2), N - 1))
    S_unc_k = np.linalg.cholesky(opt_prob.P_0)
    nc_count = 0
    for k in range(N):
        if k < N - 1:
            A_k_sparse[:, k] = opt_prob.disc["A_k"][:, :, k][opt_prob.cont_dyn["A_sp"]]
            S_unc_k = opt_prob.disc["A_err_k"][:, :, k] @ S_unc_k
            S_unc[:, k] = tril(S_unc_k).flatten()
    ...
    cvx_prob.param_dict["B_err_k"].value = opt_prob.disc["B_err_k"].reshape((neps * nu, N - 1))
    cvx_prob.param_dict["S_unc"].value = S_unc
    ...
    K_sol = cvx_prob.var_dict["K"].value.reshape((nu, neps, N - 1))
    #I_BK_sol = cvx_prob.var_dict["I_BK"].value.reshape((neps, neps, N - 1))
    ...
    S_u_sol = np.zeros((nu, neps, N - 1))
    S_u_ref = np.zeros((nu, neps, N - 1))
    for k in range(N - 1):
        if k == 0:
            S_unc_k = np.linalg.cholesky(opt_prob.P_0)
        else:
            S_unc_k = tril_to_mat(S_unc[:, k - 1])
        S_u_sol[:, :, k] = K_sol[:, :, k] @ S_unc_k
        S_u_ref[:, :, k] = K_ref[:, :, k] @ S_unc_k
    ...
    dK = 0
    for k in range(N - 1):
        dK = dK + np.linalg.norm(K_sol[:, :, k] - K_ref[:, :, k], "fro")
    ...
    return x_sol, u_sol, K_sol, cvx_prob, sol_info
