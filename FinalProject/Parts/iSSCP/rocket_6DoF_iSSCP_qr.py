...
r_std = np.array([[0.005], [0.005], [0.003]])
v_std = np.array([[0.003], [0.003], [0.001]])
theta_std = np.deg2rad(np.array([[1], [1], [1]]))
w_std = np.deg2rad(np.array([[0.1], [0.1], [0.1]]))
...
S_N = tril(np.linalg.cholesky(P_N))

terminal_bc = {"x":lambda x, x_ref : cp.concatenate([x[0:-1] - x_f[0:-1].flatten(), [0]]),
               "S":lambda S : cp.norm2(np.linalg.inv(tril_to_mat(S_N)) @ np.eye(neps - 1, neps) @ tril_to_mat_cp(S) @ np.eye(neps, neps - 1)) - 1}
...
glideslope_constraint = {"k":np.arange(N), "type":"<=", "func":lambda x, u, S, L : cp.norm2(x[0:3]) - x[2] / np.cos(glideslope_max_angle)}
...
S_max_constraint = {"k":np.arange(N), "type":"<=", "func":lambda x, u, S, L : cp.norm2(np.eye(neps - 1, neps) @ np.linalg.inv(tril_to_mat(S_0 * 3)) @ S @ np.eye(neps, neps - 1)) - 1}
...
## Specify Objective 
objective = lambda x, u, S, L, x_ref, u_ref : einsum(range(Nu), lambda k : cp.norm2(u[0:3, k]) + sqx2inv(0.99, 3) * cp.norm2(L[0:3, :, k]) + cp.abs(u[3, k]) + sqx2inv(0.99, 1) * cp.norm2(L[3, :, k])) * delta_t
...
## Solve Problem
ptr_sol = SSCP_PTR_qr_covariance(prob_6DoF, ptr_ops)
