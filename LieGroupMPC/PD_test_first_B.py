...
from LieGroupMPC.PD_test_first_problem import PD_test_problem
...
def F_func(X_k, xi_k):
    A_d = se2_Ad(se2_exp(-xi_k * prob.delta_t))
    return A_d

def F_cont_func(xi_k):
    A = -se2_ad(xi_k)
    return A
...
X_bar_nou = prob.simulate(np.zeros([3, prob.N_steps]), lambda xi_e, eta, xi, xi_bar, k : np.zeros([2, 1]), X_ref, prob.X_0)

K_p = np.array([[5e-1,    0,    0], 
                [0,    5e-0, 5e-1]])
K_d = np.zeros((2, 3))
...
X_bar_P = prob.simulate(np.zeros([3, prob.N_steps]), PD_func, X_ref, prob.X_0)
X_bar_DI = prob.simulate(np.zeros([3, prob.N_steps]), dyn_inv_func, X_ref, prob.X_0)

# Monte Carlo
N_runs = 50
start = time.perf_counter()
X_runs = prob.monte_carlo(N_runs, PD_func, X_ref)
...
Phat_k[:, :, 0] = prob.Phat_0
Q_k = lambda X : prob.delta_t ** 2 * prob.Q_body

for k in range(prob.N_steps):
    F_k = F_func(X_ref[:, :, k], prob.xi_k[:, k])
    Phat_k[:, :, k + 1] = (F_k - prob.B_d[:, :, k] @ K_p) @ Phat_k[:, :, k] @ (F_k - prob.B_d[:, :, k] @ K_p).T + Q_k(X_ref[:, :, k])

# Plot
for i in np.arange(1, prob.N_nodes, 25):
    error = np.zeros((3, N_runs))
