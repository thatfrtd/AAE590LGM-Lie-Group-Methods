...
#initial_bc = lambda x, x_ref : cp.concatenate([x[0:6] - x_0[0:6].flatten(), np.zeros((4,)), x[10:14] - x_0[10:14].flatten()])
...
r_LoS_min = 1.5 # [km]
r_LoS_max = 2.25 # [km]
LoS_distance_trigger = lambda x, u : 1e6*-np.minimum(np.linalg.norm(x[0:3]) - r_LoS_max, 0) * -np.minimum(r_LoS_min - np.linalg.norm(x[0:3]), 0)

# State convex constraints
glideslope_max_angle = np.deg2rad(75)
glideslope_constraint = {"k":np.arange(N), "type":"<=", "func":lambda x, u : cp.norm2(x[0:3]) - x[2] / np.cos(glideslope_max_angle)}
angvel_max = np.deg2rad(10)
angvel_constraint = {"k":np.arange(N), "type":"<=", "func":lambda x, u : cp.norm2(x[10:13]) - angvel_max}
angvel_max_STC = np.deg2rad(5)
...
gimbal_constraint = {"k":np.arange(Nu), "type":"<=", "func":lambda x, u : 1e3*(cp.norm2(u[0:3]) - u[0] / np.cos(gimbal_max_angle))}
...
state_nonconvex_constraints = [LoS_constraint]
...
## Specify Objective 
#objective = lambda x, u, x_ref, u_ref : cp.sum(-x[13, -1]) * m_0# + cp.sum(cp.abs(u[3, :]))
objective = lambda x, u, x_ref, u_ref : einsum(range(Nu), lambda k : cp.norm2(u[0:3, k]) + cp.abs(u[3, k])) * delta_t
