...
m_wet = 600; % [kg]
m_0 = m_dry + m_wet;
T_min = 4.97 / m_0; % [kg km / s2]
T_max = 13.26 / m_0; % [kg km / s2]
I = 150000 * (1e-3) ^ 2 / m_0; % [kg km2] ASSUMING CONSTANT MOMENT OF INERTIA
L = 3e-3; % [km] Distance from CoM to nozzle
gimbal_max = deg2rad(6); % [rad]

vehicle = Vehicle(1, L, L * 3, gimbal_max, T_min, T_max, I = I, alpha = alpha);
...
max_thrust_constraint = @(t, x, u, p) norm(u) - T_max;
max_gimbal_constraint = @(t, x, u, p) norm(u(2)) - u(1) * tan(gimbal_max);
...
min_thrust_constraint = @(t, x, u, p, x_ref, u_ref, p_ref, k) T_min - (norm(u_ref(:, k)) + u_ref(:, k)' / norm(u_ref(:, k)) * (u - u_ref(:, k)));
...
%% Specify Objective
min_fuel_angular_velocity_objective = @(x, u, p) sum(norms(u, 2, 1) / T_max + x(6, 1:Nu) .^ 2) * delta_t;
if u_hold == "ZOH"
...
if u_hold == "ZOH"
    guess.u = interp1(t_k(1:size(guess.u, 2)), guess.u', t_k(1:Nu), "previous","extrap")' + 1e-6;
elseif u_hold == "FOH"
...
prob_3DoF = DeterministicProblem(x_0, x_f, N, u_hold, tf, f, guess, convex_constraints, min_fuel_objective, scale = scale, terminal_bc = terminal_bc, nonconvex_constraints = nonconvex_constraints);
...
nexttile
plot(0:ptr_sol.converged_i, [prob_3DoF.objective(prob_3DoF.guess.x, prob_3DoF.guess.u, prob_3DoF.guess.p) * m_0, [ptr_sol.info.J] * m_0]); hold on
