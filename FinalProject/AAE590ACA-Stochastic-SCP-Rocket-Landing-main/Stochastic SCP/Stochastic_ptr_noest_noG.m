function [ptr_sol] = Stochastic_ptr_noest_noG(prob, ptr_ops, options)
arguments
    prob
    ptr_ops
...
X_ref = zeros([prob.n.x, prob.n.x, prob.N, ptr_ops.iter_max + 1]);
S_ref = zeros([prob.n.u, prob.n.x, prob.Nu, ptr_ops.iter_max + 1]);
...
for i = 1:(ptr_ops.iter_max)
    ...
    [x_ref(:, :, i + 1), u_ref(:, :, i + 1), X_ref(:, :, :, i + 1), S_ref(:, :, :, i + 1), sol_info] = solve_stochastic_ptr_convex_subproblem_no_p_noest_noG(prob, ptr_ops, x_ref(:, :, i), u_ref(:, :, i), X_ref(:, :, :, i), S_ref(:, :, :, i));
