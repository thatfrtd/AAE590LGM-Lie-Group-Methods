function success = uq_rbdo_test_Copula(level)
% SUCCESS = UQ_RBDO_TEST_COPULA(LEVEL):
...
% allowed error - relatively large error, we do not expect the solution to
% be equal to the reference due to  the introduced dependency but we would
% like the analysis to run without error
eps = 1e-1;
% Service load
Fser = 1.4622e6 ;
...
Iopts.Copula.RankCorr = [1 0.2 0.1; 0.2 1 0.1; 0.1 0.1 1];  % the Spearman corr. matrix
% Iopts.Copula.Type = 'CVine';
% Iopts.Copula.Families = {'Gumbel', 'Gaussian', 'Frank'}; 
% Iopts.Copula.Parameters = {1.5, -0.4, [0.3]};
% Iopts.Copula.Rotations = [180 0 270];
% Iopts.Copula.Structure = 1:3;
